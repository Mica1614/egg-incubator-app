...
export function useDeviceNotifications(deviceName, deviceState, preferences, enabled = true) {
  ...
  // ...and to preferences, so the offline interval sees current toggles without
  // being torn down and recreated every time preferences change identity.
  const preferencesRef = useRef(preferences);

  // Sync the refs whenever the props update
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (prevIsOnline !== isOnline) {
        // Routed through sendNotification like everything else — this path used
        // to call the toast and Firestore writers directly, which meant it
        // ignored preferences, quiet hours and the duplicate-mount throttle.
        sendNotification(
          isOnline ? NOTIFICATION_TYPES.DEVICE_ONLINE : NOTIFICATION_TYPES.DEVICE_OFFLINE,
          deviceName,
          null,
