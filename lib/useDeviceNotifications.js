  ...
  useEffect(() => {
    ...
    if (prevState.waterLow !== undefined && deviceState.waterLow !== undefined) {
      if (prevState.waterLow === false && deviceState.waterLow === true) {
        ...
        console.log(`[useDeviceNotifications] Water level went LOW`);
        sendNotification(
        ...
        console.log(`[useDeviceNotifications] Water level went OK`);
        sendNotification(
    ...
    // Also check for water property (alternative property name)
    if (prevState.water !== undefined && deviceState.water !== undefined) {
      if (prevState.water === true && deviceState.water === false) {
        // Water went from Low to OK
        console.log(`[useDeviceNotifications] Water (alt property) went OK`);
        sendNotification(
          NOTIFICATION_TYPES.WATER_OK,
          deviceName,
          null,
      ...
      } else if (prevState.water === false && deviceState.water === true) {
        // Water went from OK to Low
        console.log(`[useDeviceNotifications] Water (alt property) went LOW`);
        sendNotification(
          NOTIFICATION_TYPES.WATER_LOW,
          deviceName,
          null,
