...
import { sendNotification, sendToastNotification, NOTIFICATION_TYPES } from "@/lib/notificationService";
...
function computeIsOnline(state) {
  ...
  return (Date.now() - lastSeenMs) <= 45000;
}
...
export function useDeviceNotifications(deviceName, deviceState, preferences, enabled = true) {
  ...
  useEffect(() => {
    ...
    if (prevIsOnline !== null && prevIsOnline !== isOnline) {
      if (!isOnline) {
        // Device just went offline — always alert regardless of prefs/mute
    ...
    // ── Skip all further checks when device is offline ───────────────────
    if (!isOnline) {
      // Still update prevStateRef so we don't flood notifications on reconnect
