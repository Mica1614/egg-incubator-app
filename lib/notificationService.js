...
import { decideNotification, SEVERITY } from "@/lib/notificationPolicy.mjs";
import { readLastSent, readSettings, recordSent } from "@/lib/notificationSettings";
...
/**
 * Send a notification, subject to admission control.
 *
 * Every delivery path funnels through here so the throttle cannot be bypassed:
 * the per-type preference, the volume preset, quiet hours and the per-device
 * cooldown are all evaluated once, by lib/notificationPolicy.mjs.
 *
 * The cooldown ledger is shared through localStorage, so the dashboard and the
 * device control page mounting this hook at the same time no longer produce two
 * copies of every alert — whichever fires first claims the window.
 */
export const sendNotification = async (type, deviceName, value = null, preferences = {}) => {
  ...
  const decision = decideNotification({
    ...
    now: new Date(),
    lastSentAt: readLastSent(`${deviceName ?? "unknown"}:${type}`),
    preferences,
    settings: readSettings(),
  });

  if (!decision.allowed) {
    console.log(`[Notifications] suppressed ${type} for ${deviceName} — ${decision.reason}`);
    return;
  }

  // Claim the cooldown before any await, so two near-simultaneous callers in
  // different tabs cannot both pass the check.
  recordSent(decision.key);

  sendToastNotification(type, deviceName, value);
  // Browser push is reserved for genuine problems; routine events stay in-app.
  if (decision.severity !== SEVERITY.ROUTINE) {
