...
/**
 * Re-run threshold checks on every device state we have cached.
 * Called when the alert config changes to enabled=true so that alerts which
 * were skipped during the startup race (RTDB fired before config loaded) are
 * not silently lost.
 */
function recheckAllDevices() {
  Object.entries(latestStates).forEach(([deviceId, state]) => {
    if (state) handleDeviceState(deviceId, state);
  });
}
...
export function startDeviceMonitor() {
  ...
  adminFirestore
    ...
    .onSnapshot(
      (snap) => {
        const d = snap.data();
        const wasEnabled = emailConfig.enabled;
        emailConfig = {
        ...
        // If alerts just became enabled, re-check all cached device states.
        // RTDB fires once at startup — if config wasn't loaded yet, those checks
        // were silently skipped. Re-checking here ensures no alerts are missed.
        if (!wasEnabled && emailConfig.enabled) {
          recheckAllDevices();
        }
      },
