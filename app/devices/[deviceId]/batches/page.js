...
import {
  doc, getDoc, collection, onSnapshot, query, addDoc,
  serverTimestamp, updateDoc, deleteDoc, where, Timestamp, setDoc,
...
import {
  Plus, Egg, CheckCircle2, Trash2, AlertTriangle, Loader2, Wand2, Pencil,
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      await addDoc(collection(firestore, "egg_batches"), {
        ...
        deviceId, deviceName: nickname || deviceId,
  ...
  const handleUpdateHatch = async ({ eggsHatched, eggsFailed, hatchDate }) => {
    ...
    try {
      ...
      await setDoc(doc(firestore, "chick_inventory", `batch_${selectedHatchBatch.batchId}_${Date.now()}`), {
        ...
        hatch_date: new Date(hatchDate), deviceId, deviceName: nickname || deviceId,
                          ...
                          <div className={`rounded-2xl bg-white px-4 py-4 ring-1 ring-slate-100 shadow-sm cursor-pointer transition ${isExpanded ? "bg-sky-50/50" : "hover:ring-sky-200"}`}
                            ...
                            <div className="flex items-start justify-between gap-2">
                              <div className="min-w-0">
                                ...
                                <p className="text-[11px] text-slate-500 mt-0.5">{b.eggType} · {typeof b.totalEggs === "number" ? b.totalEggs : "—"} eggs</p>
                              </div>
